...
infolder = 'F:\Nisha\Nisha\2024\Rooks projcts\projeect3((pest identification)\pestdetectioncode\IP102 YOLOV5\test';    
...
for i = 1:n
    figure;
    subplot(1,3,1)
    m=imread(['F:\Nisha\Nisha\2024\Rooks projcts\projeect3((pest identification)\pestdetectioncode\IP102 YOLOV5\test\im' num2str(i) '.jpg']);
    imshow(m);                                                                                                                                                                                                                                                                                                     
    title('Input Image');
...
%    disp (['\phi = ' num2str(PhiOH)]);
  ...
  maxLength = max([length(f1), length(f2), length(f3), length(f4),length(f5),length(f6),length(f7),length(f8),length(f9),length(f10)]);
  f1(length(f1)+1:maxLength) = 0;
  f2(length(f2)+1:maxLength) = 0;
  f3(length(f3)+1:maxLength) = 0;
  f4(length(f4)+1:maxLength) = 0;
  f5(length(f5)+1:maxLength) = 0;
  f6(length(f6)+1:maxLength) = 0;
  f7(length(f7)+1:maxLength) = 0;
  f8(length(f8)+1:maxLength) = 0;
  f9(length(f9)+1:maxLength) = 0;
  f10(length(f10)+1:maxLength) = 0;
  Inpc=[f1; f2; f3; f4; f5 ;f6 ;f7 ;f8 ;f9 ;f10];
  Target=ones(size(Inpc,1),1);
...
[precision,specificity,Acc,f1score]= performance(Inpc,Target);
...
[precision,specificity,Acc,f1score]= ANN(Inpc,Target);
